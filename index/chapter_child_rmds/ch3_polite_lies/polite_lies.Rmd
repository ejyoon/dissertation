...
For analysis, we used a Bayesian linear mixed-effects model [`brms` package in R; @R-brms] using crossed random effects of participant, item and sample with the maximal random effects structure supported by the design [@gelman2006data; @barr2013random]. Age is plotted in bins, but was analyzed as a continuous variable, scaled and centered, in our statistical model. 

The Bayesian linear mixed model predicting "yes" responses based on participant age, context type (control vs. experimental), speaker type (honest vs. dishonest) and question type (truth-telling vs. niceness vs. meanness) showed ...
