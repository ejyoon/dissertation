...
project_path_san <- "/Users/kylemacdonald/Documents/Projects/SPEED-ACC-NOVEL"
...
## library(kableExtra)
## library(tidybayes)
## source(file.path(project_path_san,"code/helper_functions/libraries_and_functions.R"))
## source(file.path(project_path_san,"code/helper_functions/ewma_helper_funs.R"))
## source(file.path(project_path_san,"code/helper_functions/paper_helpers.R"))
## bib <- knitcitations::read.bibtex(file.path(project_path_san,"writing/manuscript/speed-acc-novel.bib"))
```
...
```{r read stimuli information speed-acc-novel}
d_gaze_stim <- read_csv(file.path(project_path_san,stimuli_path, "speed-acc-child-gaze-trial-info.csv"), col_types = cols(.default = "c"))
d_adult_ng_stim <- read_csv(file.path(project_path_san,stimuli_path, "speed-acc-adult-ng-trial-info.csv"), col_types = cols(.default = "c"))
...
d_fam_child <- read_csv(file.path(project_path_san,data_path, "familiar_words/speed_acc_child_gaze_fstshift_tidy.csv")) %>% 
  dplyr::rename(age_days = age) %>% 
  mutate(age_group = "children")

d_fam_adult <- read_csv(file.path(project_path_san,data_path, "familiar_words/speed_acc_adult_ng_fstshift_tidy.csv")) %>% 
  filter(noise_condition == "clear") %>% 
  dplyr::rename(run_date = dot,
         comments = notes) %>% 
  dplyr::select(-age, -resp_onset_type_fact, -reason_excluded) %>% 
  mutate(birthday = NA,
         age_group = "adults", 
         age_days = NA,
...
d_novel_fs <- read_csv(file.path(project_path_san,data_path, "novel_words/speed_acc_novel_shifts.csv")) %>% 
...
d_novel_tc <- read_feather(file.path(project_path_san,data_path, "novel_words/speed_acc_novel_timecourse.feather")) %>% 
  mutate(learn_block_half = ifelse(trial_num_learn_block <= 2, "first", "second"),
         trial_num_lblock_8 = case_when(
           trial_type == "exposure" & trial_num_learn_block == 1 ~ "1",
           trial_type == "test" & trial_num_learn_block == 1 ~ "2",
...
```{r linguistic stimuli length san}
...
fit_rt_fam <- brms::brm(log(rt) ~ gaze_condition + age_group + (gaze_condition + target_image|subid), 
                        ...
                        family = gaussian(),
                        silent = TRUE,
                        file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_rt_fam'))
...
fit_acc_fam <- brms::brm(correct_num ~ gaze_condition + age_group + (gaze_condition + target_image|subid), 
                         ...
                         silent = TRUE,
                         file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_acc_fam'))
...
d_gz_xsit <- read_csv(file.path(project_path_san, data_path, "novel_words/gaze_xsit_tidy_trial_level.csv"))
d_gz_xsit_tc <- read_csv(file.path(project_path_san, data_path, "novel_words/gaze_xsit_tidy_timecourse.csv"))
```
...
fit_gaze_xsit <- brms::brm(test ~ exposure * condition + (1|subid), 
                           ...
                           silent = TRUE,
                           file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_gaze_xsit_exp_test'))
...
We aimed to answer the following specific research questions: 
  ...
  1. Do children seek a disambiguating gaze cue when processing novel words?
...
Participants were native, monolingual English-learning children ($n=$ `r kids_age_table$n[2]`; `r gender_breakdown$n[3]` F) and adults ($n=$ `r n_adults_total$n[2]`; `r n_adults_gender$n[1]` F). All participants had no reported history of developmental or language delay and normal vision. `r d_excluded_e3$n[2]` adults were run but not included in the analysis because they were not native speakers of English. `r d_excluded_e3$n[1]` children were run but not included in the analysis because they did not complete more than half of the trials in the task.

#### Materials

```{r speed-acc-nov-stim}
d_nov_stim <- read_csv(file.path(project_path_san, "data/00_stimuli_information/analysis_order_sheets/speed-acc-novel-analysis-order-sheet.csv"))
...
fit_proplook_face_nov <- ss_novel_face %>% 
  ...
  brms::brm(data = ., 
            ...
            family = gaussian(),
            silent = TRUE,
            file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_proplook_face')
  )
...
fit_chance <- ss_novel_obj %>% 
  ...
  brms::brm(data = ., 
            ...
            , family = gaussian(),
            silent = TRUE,
            file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_acc_chance')
  )
...
fit_proplook_target_nov_all <- ss_novel_obj %>% 
  ...
  brms::brm(data = ., 
            ...
            , family = gaussian(),
            silent = TRUE,
            file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_proplook_targ_all')
  )
...
fit_proplook_target_nov_adults <- ss_novel_obj %>% 
  ...
  brms::brm(data = ., 
            ...
            , family = gaussian(),
            silent = TRUE,
            file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_proplook_targ_adults')
  )
...
fit_proplook_target_nov_kids <- ss_novel_obj %>% 
  ...
  brms::brm(data = ., 
            ...
            , family = gaussian(),
            silent = TRUE,
            file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_proplook_targ_kids')
  )
...
fit_exp_test_look <- ss_learning_final %>% 
  brms::brm(data = ., 
            ...
            family = gaussian(),
            silent = TRUE,
            file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_exp_test')
  )
...
fit_rt_nov <- brms::brm(log(rt) ~ (gaze_on_trial + age_category + learn_block_half)^2 + 
                        ...
                        family = gaussian(),
                        silent = TRUE,
                        file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_rt_nov_exp'))
...
fit_rt_nov_test <- brms::brm(log(rt) ~ (gaze_on_trial + age_category + learn_block_half)^2 + 
                             ...
                             family = gaussian(),
                             silent = TRUE,
                             file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_rt_nov_test'))
...
fit_acc_nov_exp <- brms::brm(correct_num ~ (gaze_on_trial + age_category + learn_block_half)^2 + (gaze_on_trial|subid),
                             ...
                             family = binomial(link = 'logit'),
                             silent = TRUE,
                             file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_acc_exp'))
...
fit_acc_nov_test <- brms::brm(correct_num ~ (gaze_condition + age_category + trial_num_learn_block)^2 + 
                              ...
                              family = binomial(link = 'logit'),
                              silent = TRUE,
                              file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_acc_test'))
...
fit_acc_nov_test_no_int <- brms::brm(correct_num ~ (gaze_condition + age_category + trial_num_learn_block) + 
                                     ...
                                     family = binomial(link = 'logit'),
                                     silent = TRUE,
                                     file = file.path(project_path_san, 'data/04_model_outputs/speed_acc_nov_fit_acc_test_noint'))
